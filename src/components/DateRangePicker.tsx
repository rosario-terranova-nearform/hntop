...
const LABEL_FORMAT = new Intl.DateTimeFormat("en", { month: "short", day: "numeric" });
...
export function DateRangePicker() {
  ...
  const selected: DateRange | undefined =
    from || to
      ? { from: from ? parseDateStr(from) : undefined, to: to ? parseDateStr(to) : undefined }
      : undefined;
  ...
  return (
    <div className="flex items-center gap-1">
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent>
          <Calendar mode="range" selected={selected} defaultMonth={selected?.from} onSelect={apply} />
        </PopoverContent>
      </Popover>
      {(from || to) && (
        <Button
